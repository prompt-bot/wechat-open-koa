...
class WechatOpen extends EventEmitter {
  ...
  async onReceiveComponentVerifyTicket (data) {
    ...
    let { AppId } = data;
    await this.cache.setComponentApp(AppId, data);
    if (this.timers.componentApp.Timer[AppId]) {
      clearTimeout(this.timers.componentApp.Timer[AppId]);
    }
    // 只启动一次
  ...
  async onRefreshAuthorizerAccessToken (data) {
    ...
    await this.cache.setAuthorizerApp(authorizer_appid, {...data, expires_at: Math.ceil(Date.now() / 1000) + expires_in - 10 * 60}); //提前10分钟
  ...
  async startFetchAuthorizerListTimer (componentAppId) {
    ...
    if (timeout > 0) {
      ...
      this.log.info(`[wechat-open-koa] [setTimeout] run this.startFetchAuthorizerListTimer(${componentAppId}) after ${timeout / 1000} seconds`);
  ...
  async startComponentAccessTokenTimer (componentAppId) {
    ...
    try {
      ...
      if (!component_access_token || expires_at <= Math.ceil(Date.now() / 1000)) {
        ...
        Object.assign(ret, { componentAppId, expires_at: Math.ceil(Date.now() / 1000) + ret.expires_in - 60 * 10  }); // 提前十分钟
      ...
      await this.cache.setComponentApp(componentAppId, {retryTimes: 0});
    } catch (err) {
      this.emit('error', err);
    ...
    this.timers.componentApp.Timer[componentAppId] = setTimeout(
      this.startComponentAccessTokenTimer.bind(this, componentAppId),
      timeout
    );
  }
  ...
  async startAuthorizerAccessTokenTimer (componentAppId, authorizerAppId) {
    ...
    try {
      ...
      if (component_access_token && (!authorizer_access_token || expires_at <= Math.ceil(Date.now() / 1000))) {
        ...
        Object.assign(ret, { AppId: componentAppId, authorizer_appid: authorizerAppId, expires_at: Math.ceil(Date.now() / 1000) + ret.expires_in - 60 * 10  }); // 提前十分钟
  ...
  autoTest (componentAppId) {
    return async (ctx, next) => {
      ...
      try {
        ...
        if ([..._.map(AUTO_TEST_MP_NAME, el => el.Username), ..._.map(AUTO_TEST_MINI_PROGRAM_NAME, el => el.Username)].includes(ToUserName)) {
          this.log.info('[wechat-open-koa] >>> 检测到全网发布测试 <<<');
          this.log.info('[wechat-open-koa] 打印消息主体:');
          // this.log.info(JSON.stringify(ctx.state.wechat));
          if (Content === AUTO_TEST_TEXT_CONTENT) {
            ctx.res.end(ctx.state.text(AUTO_TEST_REPLY_TEXT));
            this.log.info(`[wechat-open-koa] >>> 测试用例: 被动回复消息；状态: 已回复；回复内容: ${AUTO_TEST_REPLY_TEXT} <<<`);
            return;
          } else if (Content.split(':')[0] === 'QUERY_AUTH_CODE') {
            let strList = Content.split(':');
            this.log.info('[wechat-open-koa] >>> 测试用例: 主动发送客服消息');
            ctx.res.end('');
            let { component_access_token } = await this.cache.getComponentAppById(componentAppId);
            ...
            this.log.info(`[wechat-open-koa] >>> 测试用例: 主动发送客服消息；状态: 已发送；响应结果: ${JSON.stringify(ret)}；发送内容: ${content} <<<\n`
            );
          }
        } else {
          next && await next();
